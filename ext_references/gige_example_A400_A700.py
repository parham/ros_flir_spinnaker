...
def acquire_and_display_images(cam, nodemap, nodemap_tldevice):
    ...
    try:
        ...
        H2O = Humidity * numpy.exp(1.5587 + 0.06939*TAtmC -0.00027816*TAtmC*TAtmC +0.00000068455*TAtmC*TAtmC*TAtmC)
        print('H20 = ',H2O)
        print('OK')
        
        Tau = X * numpy.exp(-numpy.sqrt(Dist) * (A1 + B1 * numpy.sqrt(H2O))) + (1 - X) * numpy.exp(-numpy.sqrt(Dist) * (A2 + B2 * numpy.sqrt(H2O)))
        print('tau = ',Tau)
        print('OK')

        
        # Pseudo radiance of the reflected environment
        # r1 = ((1 – emissivity) / emissivity) * (R / (exp(B / reflectedTemp) – F))
        r1 = ((1-Emiss)/Emiss) * (R/(numpy.exp(B/TRefl)-F))
        print('r1 = ',r1)
        
        # Pseudo radiance of the atmosphere
        # r2 = ((1 – Tao) / (emissivity * tao)) * (R / (exp(B / atmosphereTemp) – F))
        r2 = ((1 - Tau)/(Emiss * Tau)) * (R/(numpy.exp(B/TAtm)-F)) 
        print('r2 = ',r2)
        
        # Pseudo radiance of the external optics
        # r3 = ((1 – extOpticsTrans) / (emissivity * Tao * extOpticsTrans)) * (R / (exp(B / extOpticsTemp) – F))
        r3 = ((1-ExtOpticsTransmission) / (Emiss * Tau * ExtOpticsTransmission)) * (R/(numpy.exp(B/ExtOpticsTemp)-F))
        print('r3 = ',r3)
        ...
        while(continue_recording):
            try:
                ...
                if image_result.IsIncomplete():
                    ...
                    # Temperature = B / ln(R / ((Radiance / emissivity) – K2) + F)
                    image_Temp = (B / numpy.log(R/((image_Radiance/Emiss/Tau)-K2) + F)) -273.15
                    ...
                    '''
                    # Displaying an image of temperature when streaming mode is et to TemperatureLinear10mK
